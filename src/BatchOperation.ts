    ...
    implements AsyncIterableIterator<[string, Element]>
{
     ...
     *                  the operation.
     */
    constructor(protected readonly client: DynamoDBClient, items: SyncOrAsyncIterable<[string, Element]>) {
        if (isIterable(items)) {
    ...
    protected movePendingToThrottled(unprocessedTables: Set<string>) {
        for (let i = this.toSend.length - 1; i > -1; i--) {
            ...
            if (unprocessedTables.has(table)) {
                (this.state[table] as ThrottledTableConfiguration<Element>).tableThrottling?.unprocessed.push(
                    attributes
                );
